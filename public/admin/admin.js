...
function displayEmailTemplates(templates) {
    ...
    if (templates.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">No email templates found.</p>';
        return;
    }
    ...
    container.innerHTML = templates.map(template => `
        <div style="background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 15px;">
                <div style="flex: 1;">
                    <h3 style="margin: 0 0 8px 0; color: #2c5f4f; font-size: 18px;">${template.name}</h3>
                    <p style="margin: 0 0 10px 0; color: #666; font-size: 14px;">${template.description}</p>
                    <p style="margin: 0; color: #999; font-size: 13px;">
                        <strong>Subject:</strong> ${template.subject}
                    </p>
                </div>
                <div style="display: flex; gap: 10px; align-items: center;">
                    <span class="status-badge ${template.active ? 'status-confirmed' : 'status-cancelled'}">
                        ${template.active ? 'Active' : 'Inactive'}
                    </span>
                    <button class="btn-small" onclick="editEmailTemplate('${template.templateId}')">Edit</button>
                </div>
            </div>

            <details style="margin-top: 15px;">
                <summary style="cursor: pointer; color: #2c5f4f; font-weight: 600; font-size: 14px; user-select: none;">
                    Available Variables (${template.availableVariables.length})
                </summary>
                <div style="margin-top: 10px; background: #f9f9f9; padding: 15px; border-radius: 6px;">
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 10px;">
                        ${template.availableVariables.map(v => `
                            <div style="font-size: 13px;">
                                <code style="background: #e0e0e0; padding: 2px 6px; border-radius: 3px; color: #d32f2f;">${v.variable}</code>
                                <span style="color: #666; margin-left: 5px;">- ${v.description}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
function showTemplateEditor(template) {
    ...
    content.innerHTML = `
        <form id="templateEditForm" onsubmit="saveEmailTemplate(event, '${template.templateId}')">
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 600;">Email Subject</label>
                <input type="text" id="templateSubject" value="${template.subject.replace(/"/g, '&quot;')}"
                       style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;" required>
            </div>

            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 600;">Email HTML Body</label>
                <div style="margin-bottom: 10px; padding: 10px; background: #f9f9f9; border-radius: 4px; border-left: 4px solid #2196f3;">
                    <p style="margin: 0 0 8px 0; font-size: 13px; color: #666;"><strong>💡 Tips:</strong></p>
                    <ul style="margin: 0; padding-left: 20px; font-size: 13px; color: #666; line-height: 1.6;">
                        <li>Use <code style="background: #e0e0e0; padding: 2px 4px; border-radius: 2px;">{{variable}}</code> for dynamic content</li>
                        <li>Use <code style="background: #e0e0e0; padding: 2px 4px; border-radius: 2px;">{{#if variable}}...{{/if}}</code> for conditional sections</li>
                        <li>Full HTML and inline CSS supported</li>
                    </ul>
                </div>
                <textarea id="templateHtmlBody" rows="20"
                          style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; font-family: 'Courier New', monospace; font-size: 13px;"
                          required>${template.htmlBody.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
            </div>

            <div style="margin-bottom: 20px;">
                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="checkbox" id="templateActive" ${template.active ? 'checked' : ''}>
                    <span style="font-weight: 600;">Template Active</span>
                </label>
                <p style="margin: 5px 0 0 28px; font-size: 13px; color: #666;">Inactive templates won't be used for sending emails</p>
            </div>
            ...
            <div style="display: flex; gap: 10px; padding-top: 20px; border-top: 1px solid #e0e0e0;">
                ...
                <button type="button" class="btn" style="flex: 1; background: #2196f3;" onclick="sendTestEmail('${template.templateId}')">Send Test Email</button>
                <button type="button" class="btn" style="background: #999;" onclick="closeTemplateEditor()">Cancel</button>
            </div>
        </form>
    `;
...
async function saveEmailTemplate(event, templateId) {
    event.preventDefault();

    const subject = document.getElementById('templateSubject').value;
    const htmlBody = document.getElementById('templateHtmlBody').value;
...
async function sendTestEmail(templateId) {
    ...
    try {
        const response = await fetch(`${API_URL}/email-templates/${templateId}/test`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${authToken}`,
                'Content-Type': 'application/json'
